...
MSnow[j][k]=2.30*out[j][k];
			else if(Tjja > -1 && Tjja <10)	//JJA平均温度在-1到10之间 
MSnow[j][k]=(0.015*Tjja)+(2.47*out[j][k]);
			else	//JJA平均温度大于等于10 
MSnow[j][k]=3.69*out[j][k];
		}
	}
	...
	*/
	...
	for (j=0;j<128;j++) {
        for (k=0;k<256;k++) {
			...
			if (Tjja <= -1)
MIce[j][k]=7.13*out[j][k];
			else if(Tjja > -1 && Tjja <10)
MIce[j][k]=(0.0067*(10-Tjja)*(10-Tjja)*(10-Tjja))+(7.16*out[j][k]);
			else
MIce[j][k]=14.80*out[j][k];
		}
	}
